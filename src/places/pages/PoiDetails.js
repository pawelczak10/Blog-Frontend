...
function DetailsPoi() {
  ...
  const fetchPOI = async () => {
    ...
    try {
      ...
      window.google.maps.event.addListener(marker, 'dblclick', (function(marker) {
        return function() {
  ...
  function printRoute(l){
    ...
    directionsDisplay = new window.google.maps.DirectionsRenderer({map: mapka,
  ...
  useEffect(() => {
    if (map!==0) {
      ...
      const mapaaa=new window.google.maps.Map(map, {
        center: { lat:parseFloat(array[0], 10), lng:parseFloat(array[1], 10)},
        zoom: 16});
        console.log("mapa",mapaaa);
        
        initialMarker = new window.google.maps.Marker({
          position:  { lat: parseFloat(array[0], 10), lng: parseFloat(array[1], 10) },
          map: mapaaa,
        });
  ...
  return (
    <><div className="leftPanel">
      ...
      <div id="googleMap" className="map" style={{width:"100%", height:"860px"}} 
  ref={setMap}
